...
/// <summary>
/// Tests for HuggingFace CDN integration — API, downloads, and model loading.
/// Registered first in Program.cs so these run before backend-specific tests.
/// No GPU accelerator needed — pure HTTP tests against the real HuggingFace API.
/// </summary>
public class HuggingFaceTests
{
    private static HttpClient CreateHFClient()
    {
    ...
    public async Task HF_GetDownloadUrl_ConstructsCorrectUrl()
    {
        ...
        Console.WriteLine("[HF_URL] All URL construction tests passed.");
    }
    ...
    [TestMethod]
    public async Task HF_DirectUrl_DownloadAndDetect()
    {
        using var http = CreateHFClient();
        var hf = new HuggingFaceClient(http);
