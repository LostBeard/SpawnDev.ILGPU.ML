...
- No API change. The canonical mechanism fix (make `AllocateWithReclaim` flush via `Flush()` not `Synchronize()`) is proposed to the SpawnDev.ILGPU lane; this ML-side fix makes the reclaim safe today for every consumer.

### 🐛 Dependency: SpawnDev.ILGPU 4.17.3 → 4.17.4 — WebGL exponential-GLSL codegen fix (Tuvok)

Bumps `SpawnDev.ILGPU` to **4.17.4**, which fixes a WebGL/GLSL backend bug where a loop containing conditionals emitted an exponentially-large shader (a 4×4 bicubic-resize loop produced a **33 MB / 500K-line** GLSL shader — its cubic-weight polynomial inlined 5,561× via a structurizer that re-emitted the loop-body continuation after every nested `if`/`else`), exhausting the Blazor WASM managed heap during compilation. This surfaced as **24 `OutOfMemoryException` / `InternalCompilerException` failures** across the ML WebGL suite; after the fix, **0** — and the WebGL lane runs 3× faster (16:38 → 5:33). WebGPU/CUDA/OpenCL/CPU/Wasm codegen is untouched. See SpawnDev.ILGPU 4.17.4 CHANGELOG.
