...
public abstract partial class MLTestBase
{
    ...
    [TestMethod]
    public async Task ZeroCopySqueezeUnsqueezeFlatten_PreservesData_AllBackends() => await RunTest(async accelerator =>
    {
        // The other metadata-only ops also hand off the buffer: input[64,64] → Relu → Unsqueeze[1,64,64] →
        // Squeeze[64,64] → Flatten[1,4096] → Relu → output. All ≥4096, single-consumer → zero-copy view.
        const int rows = 64, cols = 64, n = rows * cols;
        var rng = new Random(719);
        var x = new float[n];
        for (int i = 0; i < n; i++) x[i] = (float)(rng.NextDouble() * 4 - 2);

        var axes0 = JsonSerializer.SerializeToElement(new long[] { 0 });
        var graph = new ModelGraph
        {
            Name = "zerocopy_meta",
            Inputs = new() { new() { Name = "input", Shape = new[] { rows, cols } } },
            Outputs = new() { new() { Name = "output", Shape = new[] { 1, n } } },
            Nodes = new()
            {
                new() { OpType = "Relu", Inputs = { "input" }, Outputs = { "r1" } },
                new() { OpType = "Unsqueeze", Inputs = { "r1" }, Outputs = { "u1" }, Attributes = new() { ["axes"] = axes0 } },
                new() { OpType = "Squeeze", Inputs = { "u1" }, Outputs = { "sq1" }, Attributes = new() { ["axes"] = axes0 } },
                new() { OpType = "Flatten", Inputs = { "sq1" }, Outputs = { "fl1" }, Attributes = new() { ["axis"] = JsonSerializer.SerializeToElement(0) } },
                new() { OpType = "Relu", Inputs = { "fl1" }, Outputs = { "output" } },
            },
        };
        ...
        float worst = 0;
        for (int i = 0; i < n; i++) { float e = x[i] > 0 ? x[i] : 0; worst = MathF.Max(worst, MathF.Abs(got[i] - e)); }
        if (worst > 1e-6f)
            throw new Exception($"zero-copy Squeeze/Unsqueeze/Flatten corrupted data (worst |Δ|={worst:E3}) on {BackendName}");
        Console.WriteLine($"[ZeroCopyReshape] data intact through Squeeze/Unsqueeze/Flatten hand-off on {BackendName}");
    });
}
