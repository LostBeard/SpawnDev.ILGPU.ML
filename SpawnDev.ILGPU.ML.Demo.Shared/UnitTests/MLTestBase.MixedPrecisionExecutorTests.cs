...
public abstract partial class MLTestBase
{
    ...
    {
        // input [1,3,32,32] -> Conv(8,3,3,3)+pad1 -> [1,8,32,32] (8192, eligible) -> Relu -> Conv(4,8,3,3)+pad1
        //   -> output [1,4,32,32] (4096; graph output → stays fp32 in both runs, so we compare fp32 vs fp32).
        var graph = new ModelGraph
        {
            Name = "mixedprec_mini",
            Inputs = new() { new() { Name = "input", Shape = new[] { 1, 3, 32, 32 } } },
            Outputs = new() { new() { Name = "output", Shape = new[] { 1, 4, 32, 32 } } },
            Initializers = new()
            {
                ["c1w"] = new[] { 8, 3, 3, 3 }, ["c1b"] = new[] { 8 },
                ["c2w"] = new[] { 4, 8, 3, 3 }, ["c2b"] = new[] { 4 },
            },
            Nodes = new()
            {
                new() { OpType = "Conv", Inputs = { "input", "c1w", "c1b" }, Outputs = { "c1" }, Attributes = new() {
                ...
                new() { OpType = "Conv", Inputs = { "r1", "c2w", "c2b" }, Outputs = { "output" }, Attributes = new() {
                    ["pads"] = System.Text.Json.JsonSerializer.SerializeToElement(new[] { 1, 1, 1, 1 }),
                    ["strides"] = System.Text.Json.JsonSerializer.SerializeToElement(new[] { 1, 1 }) } },
            }
        };
        ...
        {
            ["c1w"] = pool.AllocatePermanent(RandomFloats(8 * 3 * 3 * 3, seed: 71, scale: 0.2f), new[] { 8, 3, 3, 3 }),
            ["c1b"] = pool.AllocatePermanent(RandomFloats(8, seed: 72, scale: 0.05f), new[] { 8 }),
            ["c2w"] = pool.AllocatePermanent(RandomFloats(4 * 8 * 3 * 3, seed: 73, scale: 0.2f), new[] { 4, 8, 3, 3 }),
            ["c2b"] = pool.AllocatePermanent(RandomFloats(4, seed: 74, scale: 0.05f), new[] { 4 }),
        };
        var inData = RandomFloats(1 * 3 * 32 * 32, seed: 75, scale: 1f);
        const int outN = 1 * 4 * 32 * 32;

        async Task<float[]> Run(ActivationPrecision prec)
        {
            ...
            return await hostBuf.CopyToHostAsync<float>(0, outN);
        }

        var f32 = await Run(ActivationPrecision.F32);
        var f16 = await Run(ActivationPrecision.F16);
        ...
        float tol = MathF.Max(0.02f, maxAbs * 0.03f);
        ...
        Console.WriteLine($"[MixedPrec] executor F16 matches F32 (worst |Δ|={worst:E2}, tol={tol:F4}) on {BackendName}");
    });
}
