// GLSL Kernel: glsl_kernel
// Time: 2026-03-24T00:45:07.5840000Z

#version 300 es
//
// Generated by SpawnDev.ILGPU.WebGL v2.0.0
//
precision highp float;
precision highp int;

// ============ 64-bit Emulation Library ============

// ============================================================================
// emu_i64/emu_u64 Emulation Functions (Double-Word: uvec2 where x=low, y=high)
// ============================================================================

// Create emu_i64 from int (sign-extend)
uvec2 i64_from_i32(int v) {
    uint lo = uint(v);
    uint hi = v < 0 ? 0xFFFFFFFFu : 0u;
    return uvec2(lo, hi);
}

// Create emu_u64 from uint
uvec2 u64_from_u32(uint v) {
    return uvec2(v, 0u);
}

// Convert emu_i64 to int (truncate)
int i64_to_i32(uvec2 v) {
    return int(v.x);
}

// Convert emu_u64 to uint (truncate)
uint u64_to_u32(uvec2 v) {
    return v.x;
}

// Create emu_i64 from low and high
uvec2 i64_new(uint lo, uint hi) {
    return uvec2(lo, hi);
}

// emu_i64/emu_u64 addition with carry
uvec2 i64_add(uvec2 a, uvec2 b) {
    uint lo = a.x + b.x;
    uint carry = lo < a.x ? 1u : 0u;
    uint hi = a.y + b.y + carry;
    return uvec2(lo, hi);
}

// emu_i64/emu_u64 subtraction with borrow
uvec2 i64_sub(uvec2 a, uvec2 b) {
    uint borrow = a.x < b.x ? 1u : 0u;
    uint lo = a.x - b.x;
    uint hi = a.y - b.y - borrow;
    return uvec2(lo, hi);
}

// emu_i64 negation (two's complement)
uvec2 i64_neg(uvec2 a) {
    uvec2 inv = uvec2(~a.x, ~a.y);
    return i64_add(inv, uvec2(1u, 0u));
}

// emu_u64 multiplication
uvec2 u64_mul(uvec2 a, uvec2 b) {
    uint a_lo = a.x & 0xFFFFu;
    uint a_hi = a.x >> 16u;
    uint b_lo = b.x & 0xFFFFu;
    uint b_hi = b.x >> 16u;

    uint p0 = a_lo * b_lo;
    uint p1 = a_lo * b_hi;
    uint p2 = a_hi * b_lo;
    uint p3 = a_hi * b_hi;

    uint mid = (p0 >> 16u) + (p1 & 0xFFFFu) + (p2 & 0xFFFFu);
    uint lo = (p0 & 0xFFFFu) | ((mid & 0xFFFFu) << 16u);

    uint hi = p3 + (p1 >> 16u) + (p2 >> 16u) + (mid >> 16u) + a.x * b.y + a.y * b.x;

    return uvec2(lo, hi);
}

// emu_i64 multiplication
uvec2 i64_mul(uvec2 a, uvec2 b) {
    bool neg_a = (a.y & 0x80000000u) != 0u;
    bool neg_b = (b.y & 0x80000000u) != 0u;
    uvec2 abs_a = neg_a ? i64_neg(a) : a;
    uvec2 abs_b = neg_b ? i64_neg(b) : b;
    uvec2 result = u64_mul(abs_a, abs_b);
    if (neg_a != neg_b) { result = i64_neg(result); }
    return result;
}

// Bitwise operations
uvec2 i64_and(uvec2 a, uvec2 b) {
    return uvec2(a.x & b.x, a.y & b.y);
}

uvec2 i64_or(uvec2 a, uvec2 b) {
    return uvec2(a.x | b.x, a.y | b.y);
}

uvec2 i64_xor(uvec2 a, uvec2 b) {
    return uvec2(a.x ^ b.x, a.y ^ b.y);
}

uvec2 i64_not(uvec2 a) {
    return uvec2(~a.x, ~a.y);
}

// Left shift
uvec2 i64_shl(uvec2 a, uint shift) {
    if (shift == 0u) { return a; }
    if (shift >= 64u) { return uvec2(0u, 0u); }
    if (shift >= 32u) {
        return uvec2(0u, a.x << (shift - 32u));
    }
    uint lo = a.x << shift;
    uint hi = (a.y << shift) | (a.x >> (32u - shift));
    return uvec2(lo, hi);
}

// Logical right shift
uvec2 u64_shr(uvec2 a, uint shift) {
    if (shift == 0u) { return a; }
    if (shift >= 64u) { return uvec2(0u, 0u); }
    if (shift >= 32u) {
        return uvec2(a.y >> (shift - 32u), 0u);
    }
    uint lo = (a.x >> shift) | (a.y << (32u - shift));
    uint hi = a.y >> shift;
    return uvec2(lo, hi);
}

// Arithmetic right shift
uvec2 i64_shr(uvec2 a, uint shift) {
    if (shift == 0u) { return a; }
    uint sign = a.y & 0x80000000u;
    if (shift >= 64u) {
        uint fill = sign != 0u ? 0xFFFFFFFFu : 0u;
        return uvec2(fill, fill);
    }
    if (shift >= 32u) {
        int signed_y = int(a.y);
        uint shift_amt = shift - 32u;
        int shifted = signed_y >> int(shift_amt);
        uint lo = uint(shifted);
        uint hi = sign != 0u ? 0xFFFFFFFFu : 0u;
        return uvec2(lo, hi);
    }
    uint lo_r = (a.x >> shift) | (a.y << (32u - shift));
    int signed_y_r = int(a.y);
    int shifted_r = signed_y_r >> int(shift);
    uint hi_r = uint(shifted_r);
    return uvec2(lo_r, hi_r);
}

// Signed comparisons
bool i64_eq(uvec2 a, uvec2 b) {
    return a.x == b.x && a.y == b.y;
}

bool i64_ne(uvec2 a, uvec2 b) {
    return a.x != b.x || a.y != b.y;
}

bool i64_lt(uvec2 a, uvec2 b) {
    bool a_neg = (a.y & 0x80000000u) != 0u;
    bool b_neg = (b.y & 0x80000000u) != 0u;
    if (a_neg && !b_neg) { return true; }
    if (!a_neg && b_neg) { return false; }
    if (a.y != b.y) { return a.y < b.y; }
    return a.x < b.x;
}

bool i64_le(uvec2 a, uvec2 b) {
    return i64_lt(a, b) || i64_eq(a, b);
}

bool i64_gt(uvec2 a, uvec2 b) {
    return i64_lt(b, a);
}

bool i64_ge(uvec2 a, uvec2 b) {
    return !i64_lt(a, b);
}

// Unsigned comparisons
bool u64_lt(uvec2 a, uvec2 b) {
    if (a.y != b.y) { return a.y < b.y; }
    return a.x < b.x;
}

bool u64_le(uvec2 a, uvec2 b) {
    return u64_lt(a, b) || i64_eq(a, b);
}

bool u64_gt(uvec2 a, uvec2 b) {
    return u64_lt(b, a);
}

bool u64_ge(uvec2 a, uvec2 b) {
    return !u64_lt(a, b);
}

// emu_i64 absolute value
uvec2 i64_abs(uvec2 a) {
    if ((a.y & 0x80000000u) != 0u) {
        return i64_neg(a);
    }
    return a;
}


struct struct_367760 {
    float field_0;
    uvec2 field_1;
    int field_2;
};

uniform highp int u_groupDimX; // threads per workgroup (X)
uniform highp int u_gridDimX; // number of workgroups (X)
uniform highp int u_gridDimY; // number of workgroups (Y)
uniform highp sampler2D u_param1; // buffer param[1]
uniform highp int u_param1_tileW;
uniform highp int u_param1_offset;
uniform highp int u_param1_length; // element count
uniform highp sampler2D u_param2; // buffer param[2]
uniform highp int u_param2_tileW;
uniform highp int u_param2_offset;
uniform highp int u_param2_length; // element count
uniform highp int u_param3_length; // element count
uniform highp int u_param4; // scalar param[4]
uniform highp int u_param5; // scalar param[5]
uniform highp int u_param6; // scalar param[6]
uniform highp int u_param7; // scalar param[7]

out highp float tf_out_3; // TF output for param[3]

void main() {
    int _idx = gl_VertexID;
    gl_Position = vec4(0.0, 0.0, 0.0, 1.0);
    
    // param1 is buffer (accessed via u_param1)
    // param2 is buffer (accessed via u_param2)
    // param3 is buffer (accessed via u_param3)
    int v_3;
    v_3 = u_param4;
    int v_4;
    v_4 = u_param5;
    int v_5;
    v_5 = u_param6;
    int v_6;
    v_6 = u_param7;
    
    uvec2 v_7 = uvec2(0u);
    uvec2 v_8 = uvec2(0u);
    uvec2 v_9 = uvec2(0u);
    int v_10 = 0;
    int v_11 = 0;
    int v_12 = 0;
    int v_13 = 0;
    int v_14 = 0;
    bool v_15 = false;
    bool v_16 = false;
    int v_17 = 0;
    int v_18 = 0;
    int v_19 = 0;
    int v_20 = 0;
    float v_21 = 0.0;
    int v_22 = 0;
    int v_23 = 0;
    int v_24 = 0;
    uvec2 v_25 = uvec2(0u);
    bool v_26 = false;
    uvec2 v_27 = uvec2(0u);
    bool v_28 = false;
    bool v_29 = false;
    int v_30 = 0;
    int v_31 = 0;
    bool v_32 = false;
    uvec2 v_33 = uvec2(0u);
    bool v_34 = false;
    uvec2 v_35 = uvec2(0u);
    bool v_36 = false;
    bool v_37 = false;
    int v_38 = 0;
    int v_39 = 0;
    bool v_40 = false;
    float v_41 = 0.0;
    int v_42 = 0;
    bool v_43 = false;
    uvec2 v_44 = uvec2(0u);
    bool v_45 = false;
    uvec2 v_46 = uvec2(0u);
    bool v_47 = false;
    bool v_48 = false;
    int v_49 = 0;
    int v_50 = 0;
    bool v_51 = false;
    bool v_52 = false;
    bool v_53 = false;
    int v_54 = 0;
    bool v_55 = false;
    int v_56 = 0;
    bool v_57 = false;
    bool v_58 = false;
    bool v_59 = false;
    int v_60 = 0;
    int v_61 = 0;
    bool v_62 = false;
    bool v_63 = false;
    bool v_64 = false;
    int v_65 = 0;
    bool v_66 = false;
    bool v_67 = false;
    bool v_68 = false;
    float v_69 = 0.0;
    int v_70 = 0;
    int v_71 = 0;
    int v_72 = 0;
    int v_73 = 0;
    bool v_74 = false;
    bool v_75 = false;
    bool v_76 = false;
    int v_77 = 0;
    bool v_78 = false;
    bool v_79 = false;
    bool v_80 = false;
    float v_81 = 0.0;
    float v_82 = 0.0;
    float v_83 = 0.0;
    int v_84 = 0;
    int v_85 = 0;
    // v_86 = buffer alias for param1
    v_7 = uvec2(0u);
    // v_87 = buffer alias for param2
    v_8 = uvec2(0u);
    // v_88 = buffer alias for param3
    v_9 = uvec2(0u);
    v_10 = v_4 * v_6;
    v_11 = _idx / v_10;
    v_12 = _idx % v_10;
    v_13 = v_12 % v_6;
    v_14 = v_12 / v_6;
    v_15 = v_11 >= v_3;
    if (v_15) {
    } else {
        v_16 = v_14 < v_4;
        if (v_16) {
            v_17 = v_11 * v_4;
            v_18 = v_17 * v_5;
            v_19 = v_11 * v_5;
            v_20 = v_19 * v_6;
            v_21 = float(0.0);
            v_22 = int(0);
            v_23 = v_14 * v_5;
            v_24 = v_18 + v_23;
            v_25 = uvec2(2147483648u, 4294967295u);
            v_26 = i64_ge(v_7, v_25);
            v_27 = uvec2(2147483647u, 0u);
            v_28 = i64_le(v_7, v_27);
            v_29 = v_26 && v_28;
            v_30 = i64_to_i32(v_7);
            v_31 = int(0);
            v_32 = v_30 == v_31;
            v_33 = uvec2(2147483648u, 4294967295u);
            v_34 = i64_ge(v_8, v_33);
            v_35 = uvec2(2147483647u, 0u);
            v_36 = i64_le(v_8, v_35);
            v_37 = v_34 && v_36;
            v_38 = i64_to_i32(v_8);
            v_39 = int(0);
            v_40 = v_38 == v_39;
            v_41 = v_21;
            v_42 = v_22;
            for (int _loop0 = 0; _loop0 < 100000; _loop0++) {
                v_43 = v_42 < v_5;
                if (!v_43) {
                    break;
                }
                v_60 = v_24 + v_42;
                // String: 32-bit index out of range
                v_61 = int(0);
                v_62 = v_60 >= v_61;
                v_63 = v_60 < v_30;
                v_64 = v_62 && v_63;
                v_65 = int(0);
                v_66 = v_60 == v_65;
                v_67 = v_66 && v_32;
                v_68 = v_64 || v_67;
                // String: X index out of bounds
                int v_89 = int(v_60); // LEA into param1
                v_69 = float(texelFetch(u_param1, ivec2((int(v_89) + u_param1_offset) % u_param1_tileW, (int(v_89) + u_param1_offset) / u_param1_tileW), 0).r);
                v_70 = v_42 * v_6;
                v_71 = v_20 + v_70;
                v_72 = v_71 + v_13;
                // String: 32-bit index out of range
                v_73 = int(0);
                v_74 = v_72 >= v_73;
                v_75 = v_72 < v_38;
                v_76 = v_74 && v_75;
                v_77 = int(0);
                v_78 = v_72 == v_77;
                v_79 = v_78 && v_40;
                v_80 = v_76 || v_79;
                // String: X index out of bounds
                int v_90 = int(v_72); // LEA into param2
                v_81 = float(texelFetch(u_param2, ivec2((int(v_90) + u_param2_offset) % u_param2_tileW, (int(v_90) + u_param2_offset) / u_param2_tileW), 0).r);
                v_82 = v_69 * v_81;
                v_83 = v_41 + v_82;
                v_84 = int(1);
                v_85 = v_42 + v_84;
                v_41 = v_83;
                v_42 = v_85;
                continue;
            }
            v_44 = uvec2(2147483648u, 4294967295u);
            v_45 = i64_ge(v_9, v_44);
            v_46 = uvec2(2147483647u, 0u);
            v_47 = i64_le(v_9, v_46);
            v_48 = v_45 && v_47;
            // String: 32-bit index out of range
            v_49 = i64_to_i32(v_9);
            v_50 = int(0);
            v_51 = _idx >= v_50;
            v_52 = _idx < v_49;
            v_53 = v_51 && v_52;
            v_54 = int(0);
            v_55 = _idx == v_54;
            v_56 = int(0);
            v_57 = v_49 == v_56;
            v_58 = v_55 && v_57;
            v_59 = v_53 || v_58;
            // String: X index out of bounds
            int v_91 = int(_idx); // LEA into param3
            tf_out_3 = v_41;
        } else {
        }
    }
    return;
}
