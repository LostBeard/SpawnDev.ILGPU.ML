...
| PMT `DA3_NativeAspect_Preprocess_MatchesReference` (all backends) | `DepthResizeMode.NativeAspect` / `ImagePreprocessKernel.ForwardNativeAspect` reproduces the OFFICIAL DA3 preprocessing (real OpenCV INTER_AREA/INTER_CUBIC in the reference) to within one uint8 LSB. Bilinear-at-the-same-size is the built-in negative control. |
| PMT `DA3_NativeAspect_Pipeline_MatchesOrt` | `DepthEstimationPipeline { ResizeMode = NativeAspect }` from raw pictures vs ORT: single view through capture/replay (incl. a larger picture forcing the stable-buffer regrow), joint multi-view depth + poses + intrinsics, and the mixed-shape refusal. |
| PMT `DA3_MultiView_Profile` | Per-op GPU attribution (`PerOpSync`) of the slow large joint forward vs a fast one, plus the WebGPU replay encode/submit/GPU-wait split. Writes `profile-<case>.json` to the sink. A measurement, not a gate. |
